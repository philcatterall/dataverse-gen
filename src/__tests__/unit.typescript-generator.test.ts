...
jest.mock("../CodeWriter");
jest.mock("../TemplateProvider");

describe("TypescriptGenerator", () => {
  let model;
  let codeWriter: jest.Mocked<CodeWriter>;
  let templateProvider: jest.Mocked<TemplateProvider>;
  ...
  beforeEach(async () => {
    model = await getModel(defaultOptions);
    codeWriter = { createSubFolder: jest.fn(), write: jest.fn() } as any;
    templateProvider = {
    ...
    typescriptGenerator = new TypescriptGenerator(model, codeWriter, templateProvider, defaultOptions, jest.fn());
  });

  it("generates code", async () => {
    await typescriptGenerator.generate();
